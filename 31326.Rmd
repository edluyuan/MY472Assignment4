...
combined_data$officer_defined_ethnicity <- ifelse(combined_data$officer_defined_ethnicity == "Mixed", "Other", combined_data$officer_defined_ethnicity)
```
...
proportion_table <- combined_data %>%
...
# View the result
ggplot(outcome_table, aes(x = officer_defined_ethnicity, y = proportion, fill = outcome)) +
...
```{r Normalization}
combined_data <- combined_data %>%
  mutate(date = as.Date(ymd_hms(datetime)), year = year(date)) %>% ungroup()
...
daily_counts <- merge(daily_counts, census_race, by = "officer_defined_ethnicity")
# Calculate standardized counts
daily_counts$n <- (daily_counts$n / daily_counts$total) * 100

```

```{r}
ggplot(daily_counts, aes(x = date, y = n, color = officer_defined_ethnicity)) +
  geom_line() +
  scale_x_date(
...
```{r}
daily_counts_age <- combined_data %>%
...
census_age <- census_age %>%
  ...
  summarise(total = sum(count))
# Merge with search number to get a weighted search rates
daily_counts_age <- merge(daily_counts_age, census_age, by = "age_range")
# Calculate standardized counts
daily_counts_age$n <- (daily_counts_age$n / daily_counts_age$total) * 100

```

```{r}

ggplot(daily_counts_age, aes(x = date, y = n, color = age_range)) +
  geom_line() +
  scale_x_date(
    breaks = date_breaks("3 months"),
    labels = date_format("%b %Y")
...
```{r}
ggplot(daily_counts_gender, aes(x = date, y = n, color = gender)) +
  geom_line() +
  scale_x_date(
    breaks = date_breaks("3 months"),
    labels = date_format("%b %Y")
...
```{r}
for (location in combined_data$location) {
  if (is.na(location)) {
    print(location)
  }
}
for (operation in combined_data$operation_name) {
  if (!is.na(operation)) {
    print(operation)
  }
}
...
ggplot(final_arrest_master, aes(
    x = white_arrest_rate,
...
# Merge the reshaped hit rates and total searches
final_master <- merge(hitRate_master, totSearch_master, by = c("force", "minority_race"))
```
...
```{r}
min_counts <- combined_data %>%
  count(time, officer_defined_ethnicity) 
# Merge with search number to get a weighted search rates
min_counts <- merge(min_counts, census_race, by = "officer_defined_ethnicity")
# Calculate standardized counts
min_counts$n <- (min_counts$n / min_counts$total) * 100
min_counts$time <- as.POSIXct(min_counts$time, format = "%H")
```

```{r}
ggplot(min_counts, aes(x = time, y = n, color = officer_defined_ethnicity)) +
  geom_line() +
  scale_x_datetime(
...
```{r}
check <- combined_data %>%  filter(outcome == "Caution (simple or conditional")
unique(combined_data$outcome)
```
